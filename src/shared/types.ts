...
export interface ProofAnnotation {
  ...
  // rect / ellipse / highlight: x, y, width, height (top-left + size)
  x?: number;
  y?: number;
  width?: number;
  height?: number;
  // arrow: tail (x1,y1) -> head (x2,y2)
  x1?: number;
  y1?: number;
  x2?: number;
  y2?: number;
...
export type ExtensionMessage =
  ...
  /** Ask the webview to burn annotations into an image on a canvas (the
   * webview is the only place with full drawing/typography support — no native
   * image deps in the extension host). Answered with "annotateResult". */
  | {
      type: "annotateImage";
      requestId: string;
      image: string;
